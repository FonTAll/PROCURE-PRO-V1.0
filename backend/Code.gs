...
const GLOBAL_SHEETS_CONFIG = {
    // ผู้ใช้งานระบบ (Users & Permissions)
    'Users': ['id', 'employeeId', 'idCard', 'name', 'role', 'permissions', 'position', 'email', 'avatar', 'isDev', 'status'],
    
    // จัดการขอซื้อขอจ้าง (PR & PO Management)
    'PurchaseRequests': ['id', 'prNumber', 'requestDate', 'requesterId', 'department', 'totalAmount', 'status', 'approvedBy', 'remarks', 'items'],
    'PurchaseOrders': ['id', 'poNumber', 'prId', 'vendorId', 'poDate', 'deliveryDate', 'totalAmount', 'status', 'approvedBy', 'items'],
    
    // บริหารผู้ขาย (Vendor Hub)
    'Vendors': ['id', 'vendorCode', 'name', 'taxId', 'contactPerson', 'email', 'phone', 'address', 'category', 'status', 'slaRating', 'createdAt'],
    'StrategicContracts': ['id', 'contractNo', 'vendorId', 'title', 'startDate', 'endDate', 'value', 'status', 'attachmentUrl'],
    'SCARs': ['id', 'scarNo', 'vendorId', 'issueDate', 'type', 'description', 'rootCause', 'correctiveAction', 'status', 'closedDate'],
    
    // คลังสินค้าและตรวจสอบคุณภาพ (Inventory & MES)
    'Inventory': ['id', 'itemCode', 'itemName', 'category', 'quantity', 'unit', 'minStock', 'maxStock', 'location', 'lastUpdated'],
    'GoodsReceiving': ['id', 'grNumber', 'poId', 'receiveDate', 'receiverId', 'status', 'notes', 'items'],
    'QualityInspections': ['id', 'qiNumber', 'grId', 'inspectorId', 'inspectDate', 'result', 'remarks', 'status'],
    
    // การประเมินคุณภาพผู้ขาย (Evaluations)
    'VendorEvaluations': ['id', 'evalNumber', 'vendorId', 'evaluatorId', 'period', 'scoreQuality', 'scoreDelivery', 'scoreService', 'scorePrice', 'totalScore', 'grade', 'status', 'remarks'],
    
    // การตรวจสอบและการปฏิบัติตามข้อกำหนด (Audit & Compliance)
    'Audits': ['id', 'auditNo', 'vendorId', 'auditorId', 'planDate', 'actualDate', 'type', 'score', 'findingCount', 'status', 'reportUrl'],
    
    // บริหารความเสี่ยง (Risk Management)
    'RiskAssessments': ['id', 'assessmentNo', 'vendorId', 'riskType', 'probability', 'impact', 'riskLevel', 'mitigationPlan', 'status', 'assessedDate'],
    
    // การเงินและภาษี (Finance & VAT)
    'FinanceVAT': ['id', 'invoiceNo', 'poId', 'vendorId', 'amount', 'vatAmount', 'totalAmount', 'invoiceDate', 'dueDate', 'paymentStatus', 'paidDate'],
    
    // ข้อมูลหลัก (Master Data)
    'MasterItems': ['id', 'itemCode', 'name', 'description', 'category', 'unit', 'unitPrice', 'status', 'createdAt'],
    'Categories': ['id', 'code', 'name', 'description', 'status'],
    
    // ปฏิทินและกิจกรรม (Calendar)
    'CalendarEvents': ['id', 'date', 'time', 'title', 'type', 'priority', 'status', 'color', 'createdAt'],
    
    // บันทึกการเข้าใช้งาน (Access Logs)
    'AccessLogs': ['id', 'userId', 'action', 'module', 'timestamp', 'ipAddress', 'details'],
